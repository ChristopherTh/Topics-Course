...
Motivation % K only gicvves discrete choice of degree of smoothnes notation erklärend j supressed for ease of notation
\begin{align*} 
...
\begin{frame}{Penalized smoothing spline - general new } %x twice differntiable
\begin{align*}
...
truncated polynomial spline %kjurzes bild an der tafel machen 
\begin{align*}
f(t)=\beta_0+\beta_2t+\beta_2t^2+...+\sum_{l=q}^L \beta_{pl}(t-\tau_l)^p_+ \\ 
\end{align*}
penalized smoothing spline %minimize wrts to betas
\begin{align*}
PENN^{(j)}_{\lambda,m}=\sum_{i=1}^n (y_i^{(j)}-f^{(j)}(t_i))^2+\lambda PEN^{(j)}_m \int_{t_1}^{t_n}D^mf(s)^2 ds %wie bestimmt man lambda und m und knots?
\end{align*}
\end{frame}
...
\begin{frame}{Estimating the model} %problem aufschreiben um zu zeigen das es nicht so einfahc ist wie aes ausieht, different methods available
\begin{itemize}
